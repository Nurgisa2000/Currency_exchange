...
def base_currency_function(message: types.Message):
    ...
    rate = message.text #.replace(",", "").replace(".","").replace("","")
    ...
    bot.reply_to(message, f"Обмен {rate} KZT началься...")
    ...
    # text = (
    #     f"{main_pars('kgs', rate)}\n"
    #     f"{main_pars('rub', rate)}\n"
    #     f"{main_pars('usd', rate)}\n"
    #     f"{main_pars('eur', rate)}\n"
    #     f"{main_pars('uah', rate)}\n"
    # )

    # bot.send_message(message.chat.id, output) #text)
